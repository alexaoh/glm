...
ggplot(all, aes(x = EDAT, y = Count)) + 
  ...
  geom_text(aes(label=Count, group= SEXE), vjust = 1.6, color="white", 
...
```{r}
# Dette er det du hadde ;) Tror ikke dette stemmer helt, siden dette viser antall rader, men vi må vel telle opp i RECOMPTE?
# Det har jeg gjort i hvert plot under dine. Hva tenker du?
## Ja du har rett. Fjerner mine plot.
...
ggplot(data = week.data, aes(x = Week, y = Count)) +
  labs(x = "Week", y = "Count") +
  geom_col(aes(fill = SEXE), position = "dodge") +
...
# Not sure why the numbers aren't aligning properly in this case. 
...
Try models: Multinomial, Poisson and Binomial (for each row in a contingency table) (As mentioned in lecture)

### EDA before Model Fit

```{r}
plot(covid.data$PAUTA, covid.data$RECOMPTE)
```
...
```{r}
model1 <- lm(RECOMPTE ~ SEXE + EDAT + DATA + PAUTA, data = covid.data) #Blir dette rett?
summary(model1)
par(mfrow = c(2,2))
plot(model1, ask = F)
par(mfrow = c(1,1))
...
Looking at the plots above, this is obviously not a good model. Moreover, The $R^2$ is under $10\%$ which is very bad. 

### Generalized Linear Model

```{r}
model2 <- glm(RECOMPTE ~ SEXE + EDAT + DATA + PAUTA, family = poisson(link = "log"), data = covid.data)
summary(model2)
par(mfrow = c(2,2))
plot(model2, ask = F)
par(mfrow = c(1,1))
...
```{r}
# Vanskelig å skjønne hva som blir Y i modellen vår. Tenker kanskje å 
# bruke måneden som en kategorisk variabel, og summere opp RECOMPTE.
# Da kan vi se om vaksinasjonsstatus har noe og si for totalt antall 
# kritiske pasienter i en spesifikk måned.
...
```{r}
model4 <- lm(RECOMPTE ~ SEXE + PAUTA + Month, data = covid.data.aggr)
summary(model4)
par(mfrow = c(2,2))
plot(model4, ask = F)
```
...
Har prøvd en del forskjellige modeller, men de blir aldri gode nok til å 
aksepteres. Jo flere interactions vi tillater, jo bedre blir den, men også kaotisk.
Gir det mening å gruppere i måneder? Tror vi er nødt til å gjøre noe i den duren
for å ha nok eksempler til å fitte dataen på. Alle modellene er overdispersed,
men jeg vet ikke hva vi kan gjøre med det? Er ikke sikkert det forventes at modellen
skal være fantastisk, idk.
